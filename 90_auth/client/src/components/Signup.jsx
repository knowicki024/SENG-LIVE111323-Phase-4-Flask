...
function Signup({updateUser}) {
  ...
  const handleClick = () => setSignUp((signUp) => !signUp)
    //build the authentication form with formik
      //formSchema and use yup to make client side validations

   const formSchema = yup.object().shape({
      name: yup.string().required("Please enter username! :-D"),
      // email: yup.string().required("You need to enter email address")
    })
  ...
  const formik = useFormik({ 
      ...
      onSubmit:(values) => {
            ...
            .then(resp => resp.json())
            .then(user => {
              updateUser(user) //(updateUser is passed down from app 
                               //through props)
              history.push('/') //redirect to the Home page.
              console.log("hello")
              console.los("current user in auth--", user) //successful POST 
